...
function getFirstLocalStorageItem<T, E>(
  key: string,
  legacyKeys: readonly string[],
  schema: Schema.Codec<T, E>,
): { value: T; matchedKey: string } | null {
  for (const candidateKey of [key, ...legacyKeys]) {
    const item = isomorphicLocalStorage.getItem(candidateKey);
    if (!item) continue;
    try {
      return {
        value: decode(schema, item) as T,
...
const LOCAL_STORAGE_CHANGE_EVENT = "samscode:local_storage_change";
...
export function useLocalStorage<T, E>(
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    try {
      const item = getFirstLocalStorageItem(key, legacyKeys, schema);
      if (!item) return initialValue;
      if (item.matchedKey !== key) {
        setLocalStorageItem(key, item.value, schema);
      }
      return item.value;
    } catch (error) {
  ...
  useEffect(() => {
    if (prevKeyRef.current !== key) {
      ...
      try {
        const newValue = getFirstLocalStorageItem(key, legacyKeys, schema);
        if (!newValue) {
          setStoredValue(initialValue);
          return;
        }
        if (newValue.matchedKey !== key) {
          setLocalStorageItem(key, newValue.value, schema);
        }
