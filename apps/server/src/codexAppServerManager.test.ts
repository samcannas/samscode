...
import { ApprovalRequestId, ThreadId } from "@samscode/contracts";
...
describe("startSession", () => {
  it("enables Codex experimental api capabilities during initialize", () => {
    expect(buildCodexInitializeParams()).toEqual({
      clientInfo: {
        name: "samscode_desktop",
        title: "Sam's Code Desktop",
        version: "0.1.0",
      },
      capabilities: {
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Sam's Code. Upgrade to v0.37.0 or newer and restart Sam's Code.",
        );
      });
