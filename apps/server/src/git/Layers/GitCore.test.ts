...
it.layer(TestLayer)("git integration", (it) => {
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "samscode/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "samscode/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "samscode/tmp-working" });

        const renamed = yield* renameGitBranch({
          cwd: tmp,
          oldBranch: "samscode/tmp-working",
          newBranch: "samscode/feat/session",
        });

        expect(renamed.branch).toBe("samscode/feat/session-1");
        const branches = yield* listGitBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "samscode/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "samscode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("samscode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "samscode/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "samscode/feat/session-1" });
        yield* createGitBranch({ cwd: tmp, branch: "samscode/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "samscode/tmp-working" });
        ...
        expect(renamed.branch).toBe("samscode/feat/session-2");
      }),
    );
  ...
  describe("GitCore", () => {
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:samcannas/samscode.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:samcannas/samscode.git/",
          });
    ...
    it.effect("truncates oversized staged patches instead of failing commit context prep", () =>
      Effect.gen(function* () {
        const core = yield* makeIsolatedGitCore({
          execute: (input) => {
            if (input.operation === "GitCore.prepareCommitContext.stagedSummary") {
              return Effect.succeed({ code: 0, stdout: " file.ts | 1 +\n", stderr: "" });
            }
            if (input.operation === "GitCore.prepareCommitContext.stagedPatch") {
        ...
        const context = yield* core.prepareCommitContext(process.cwd());

        expect(context).not.toBeNull();
        expect(context?.stagedPatch).toContain("[diff truncated]");
      }),
    );

    it.effect("truncates oversized range diffs instead of failing PR context prep", () =>
      Effect.gen(function* () {
        const core = yield* makeIsolatedGitCore({
          execute: (input) => {
            if (input.operation === "GitCore.readRangeContext.log") {
              return Effect.succeed({ code: 0, stdout: "abc123 change\n", stderr: "" });
            }
            if (input.operation === "GitCore.readRangeContext.diffStat") {
              return Effect.succeed({ code: 0, stdout: " file.ts | 1 +\n", stderr: "" });
            }
            if (input.operation === "GitCore.readRangeContext.diffPatch") {
              expect(input.outputMode).toBe("truncate");
              return Effect.succeed({
                code: 0,
                stdout: "diff --git a/file.ts b/file.ts\n@@\n+hello\n",
        ...
        const context = yield* core.readRangeContext(process.cwd(), "main");

        expect(context.diffPatch).toContain("[diff truncated]");
      }),
    );
  });
});
