...
function toAgentCatalogId(value: string): string {
  return value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9\s_-]/g, "")
...
function parseMarkdownFrontmatter(contents: string): {
...
function readSupportedProviders(
  frontmatter: Record<string, ParsedFrontmatterValue>,
): ProviderKind[] {
...
function parseCatalogMarkdownFile(input: {
  ...
  const id = toAgentCatalogId(rawId);
  ...
  const name =
    /[\sA-Z]/.test(rawName) || rawName.includes("(")
      ? rawName
      : humanizeAgentId(toAgentCatalogId(rawName));
